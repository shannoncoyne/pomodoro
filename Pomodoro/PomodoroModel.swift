...
class PomodoroModel: PomodoroDelegate {
    ...
    func userInputIsValid(input: String, field: String) {
        ...
        default: // intervalLength and breakLength. validate int values.
            let val = input.toInt() // returns optional int
    ...
    func getPomodoroFromIndexPathRow(row: Int) -> Pomodoro? {
        var error: NSError?
        if let p = managedContext.existingObjectWithID(pomodori[row].objectID, error: &error) {
    ...
    private func save() {
        ...
        if !managedContext.save(&error) {
            println("Could not save changes \(error), \(error?.userInfo)")
        }
    }
    
    private func loadPomodori() {
        println("Loading pomodori")
        let request = NSFetchRequest()
        request.entity = entity
        
        var error: NSError?
        
        var objects = managedContext.executeFetchRequest(request, error: &error) as? [NSManagedObject]
        
        if let obs = objects {
            pomodori = obs
        } else {
            println("No pomodoro objects found to load")
        }
    }
    ...
    private func notifyObservers(#success: Bool) {
